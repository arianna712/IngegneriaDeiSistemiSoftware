...
class Firefly_1 ( name: String, scope: CoroutineScope, isconfined: Boolean=false, isdynamic: Boolean=true ) : 
	...
	override fun getBody() : (ActorBasicFsm.() -> Unit){
		...
		return { //this:ActionBasciFsm
				...
				state("flash") { //this:State
					...
					transition(edgeName="t01",targetState="flash",cond=whenDispatch("sync"))
				}	 
				state("handlesync") { //this:State
					action { //it:State
						if( checkMsgContent( Term.createTerm("sync(T)"), Term.createTerm("sync(T)"), 
								...
								CommUtils.outgreen("$name synchronized at Timer=$Timer")
						}
						//genTimer( actor, state )
					}
					//After Lenzi Aug2002
